...
struct _block *findFreeBlock(struct _block **last, size_t size) 
{
 ...
 // \TODO Put your Best Fit code in this #ifdef block   
   ...
   struct _block *Best_Fit = NULL;
    size_t minimum = (size_t)INT_MAX;
   
           /* 100000000*/
 while (curr) 
   {
      
      if(curr && (curr->size >= size && curr->free)&& curr->size < minimum)
      {
         Best_Fit = curr;
         minimum = curr->size;
 ...
 while (curr) 
   {
      if(curr && (curr->size >= size && curr->free)&& curr->size > maximum )
      {
         Worst_Fit = curr;
         maximum = curr->size;
      }
...
void *malloc(size_t size) 
{
   ...
   {
      size_t result = (next->size) - size;

      if(result >= (sizeof(struct _block ) + 4))
      {
      
       struct _block *Original_End =next->next;
       size_t next_Original = next->size;
       
       next->size = size;
       next->next = next + (unsigned char)next->size + (unsigned char)sizeof(struct _block);
       next->free = false;

       Next_Block = next->next;
       Next_Block->size = next_Original - (unsigned char)size - (unsigned char)sizeof(struct _block);
       Next_Block->free = true;
       Next_Block->next = Original_End;
...
void free(void *ptr) 
{
   ...
   struct _block *Current_Block = heapList;
   struct _block *New_End = NULL;

   while(Current_Block)
   {
      if(Current_Block && (Current_Block)->next && Current_Block->free && (Current_Block->next)->free)
      {
         Current_Block->size = Current_Block->size + (Current_Block->next)->size + sizeof(struct _block);
         Current_Block->free = true;
         New_End = (Current_Block->next)->next;
         Current_Block->next = New_End;
         Current_Block = heapList;
