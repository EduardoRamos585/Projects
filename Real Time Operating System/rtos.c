...
int main(void)
{
    ...
    ok =  createThread(idle, "Idle", 15, 512);  // 0
    //ok &=  createThread(idle2, "Idle2", 15, 512);


    // Add other processes
     ok &= createThread(lengthyFn, "LengthyFn", 12, 1024); // 1
     ok &= createThread(flash4Hz, "Flash4Hz", 8, 512);  // 2
     ok &= createThread(oneshot, "OneShot", 4, 1536); //3
    ...
    ok &= createThread(errant, "Errant", 12, 512); //8
    ok &= createThread(shell, "Shell", 12, 4096); //9
